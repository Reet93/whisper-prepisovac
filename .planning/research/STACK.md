...
**Confidence:** HIGH (all core libraries verified against PyPI + official docs)
...
pip install torch==2.11.0  # Windows: use CUDA wheel from https://pytorch.org/get-started/locally/
...
**GPU auto-detection (applies to both platforms):**
```python
import torch
...
def get_device() -> str:
    ...
    return "cpu"
```
Pass the result as the `device` argument to `whisper.load_model(name, device=get_device())`.

**PyInstaller: bundling the Whisper model:**
...
**i18n switch at runtime:**
```python
import gettext, locale

LOCALES = {"cs": "cs_CZ", "en": "en_US"}

def set_language(lang: str):
    translation = gettext.translation(
        "messages",
        localedir=bundled_locale_dir(),
        languages=[LOCALES[lang]],
        fallback=True,
    )
    translation.install()  # installs _() into builtins
```
Rebuild all UI strings after the switch by re-rendering widgets or storing `StringVar` references.
...
*Stack research for: Whisper Přepisovač — cross-platform desktop audio transcription app*
*Researched: 2026-03-28*
